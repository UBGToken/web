...
contract TetherToken is Pausable, StandardTokenWithFees, BlackList {
    ...
    // depreanime current contract in favour of a new one
    function depreanime(address _upgradedAddress) public onlyOwner {
        require(_upgradedAddress != address(0));
        deprecated = true;
        upgradedAddress = _upgradedAddress;
        Depreanime(_upgradedAddress);
    }

    // depreanime current contract if favour of a new one
    function totalSupply() public constant returns (uint) {
        if (deprecated) {
    ...
    event Depreanime(address newAddress);
