...
// File: contracts/NCFToken.sol

pragma solidity 0.6.12;

// NCFToken with Governance.
contract NCFToken is ERC20("NCFToken", "NCF"), Ownable {
...
// File: contracts/NCFFarm.sol

pragma solidity ^0.6.0;

contract NCFFarm is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        NCFToken rewardToken;        // Address of reward token contract.
    ...
    IERC20 public referralToken;  // NCF token for check referral program

    uint256 public stakeAmountLv1;    // Minimum stake NCF token condition level1 for referral program.
    uint256 public stakeAmountLv2;    // Minimum stake NCF token condition level2 for referral program.
...
constructor(
    ...
    function _setAllocPoints(NCFToken _rewardToken, uint256 _allocPoint) internal onlyOwner {
    ...
    function claimReward(uint256 _pid) public {
        require(status == true, "NCFFarmer::withdraw: can not claim reward");
        updatePool(_pid);
        _harvest(_pid);
    }
    ...
    // Deposit LP tokens to NCFFarmer.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public {
        require(_amount > 0, "NCFFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from NCFFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(status == true, "NCFFarmer::withdraw: can not withdraw");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "NCFFarmer::withdraw: not good");
    ...
    function totalLock(NCFToken _rewardToken) public view returns (uint256) {
