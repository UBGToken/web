...
export class SmcService {
  ...
  static contractNCF: ContractToken;
  ...
  static async initialize(store: Store): Promise<any> {
    ...
    try {
      ...
      const contractNCF = new this.web3.eth.Contract(this.configs['SMC_NCF_ABI'], this.configs['SMC_NCF_ADDRESS']);
      ...
      this.contractNCF = {
        balanceOf: () => contractNCF.methods.balanceOf(this.address)
          .call({ from: this.address })
          .then((value: any) => +value / 1000000000000000000),
        approve: (amount) => contractNCF.methods.approve(this.configs['SMC_MAIN_ADDRESS'], NumberUtils.cryptoConvertToStringAmount(amount, 18))
          .send({ from: this.address }),
        allowance: () => contractNCF.methods.allowance(this.address, this.configs['SMC_MAIN_ADDRESS'])
          .call({ from: this.address })
          .then((value: any) => +value / 1000000000000000000),
      }
  ...
  static async fetchSMCWallet(store: Store, address: string = this.address) {
    try {
      if (address) {
        ...
        const balanceUSDT = await this.contractUSDT.balanceOf();
        const balanceNCF = await this.contractNCF.balanceOf();

        store.dispatch({ type: SET_SMC_WALLET_DATA, address, balanceUSDT, balanceNCF });
      } else {
